...
class Logs {
    ...
    function __construct() {
        ...
        add_action( 'template_redirect', array(Logs::class, 'update_notification_status') );
        add_action( 'voynotif/template/footer', array(Logs::class, 'add_pixel') );
            ...
            foreach( $logs as $log ) {
                ...
                ob_start(); ?>
                    <ul>
                        ...
                        <li><?php _e('Status: ', 'notifications-center'); ?><?php echo Logs::get_status_title( $log->status ); ?></li>
                    </ul>
                <?php $value = ob_get_clean();
                $logs_to_export[] = array(
                    'name'  => $log_date->format('d/m/Y à G:i'),
                    'value' => __('Subject: ', 'notifications-center').$log->subject.' | '.Logs::get_status_title( $log->status ),
                );                
            }
        }
    ...
    public static function add_log( $args = array() ) {
        ...
        if( $result === 1 ) {
            return Logs::get_log($wpdb->insert_id);
        }
