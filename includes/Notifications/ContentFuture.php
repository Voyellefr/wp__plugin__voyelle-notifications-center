 ...
 */
class ContentFuture extends NotificationType {
    ...
    function __construct() {
        ...
        $this->name = 'content_future';
        $this->label = __( 'Planified content', 'notifications-center' );
        $this->tags = array(
            'content'
        );
     ...
     */
    function init() {
        add_action( 'transition_post_status', array( $this, 'content_future' ), 10, 3 );        
    }
    ...
    /**
     * Set up custom hooks to check all cpt futureed
     * 
     * @author Floflo
     * @since 0.9
     * @update 2016-06-02
     */
    function content_future( $new_status, $old_status, $post ) {

        if ( 'future' == $new_status AND $old_status != 'future' ) {
