...
namespace Voyelle\NotificationsCenter\Notifications;

use Voyelle\NotificationsCenter\Framework\NotificationType;

/**
 * NOTIFICATION Pending Content
 */
class ContentPublish extends NotificationType {
    ...
    function __construct() {
        ...
        $this->name = 'content_publish';
        $this->label = __( 'Published content', 'notifications-center' );
        $this->tags = array(
            'content'
        );
     ...
     */
    function init() {
        add_action( 'transition_post_status', array( $this, 'content_publish' ), 10, 3 );        
    }
    ...
    /**
     * Set up custom hooks to check all cpt published
     * 
     * @author Floflo
     * @since 0.9
     * @update 2016-06-02
     */
    function content_publish( $new_status, $old_status, $post ) {

        if ( 'publish' == $new_status AND $old_status != 'publish' ) {
