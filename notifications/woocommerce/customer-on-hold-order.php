<?php
class VOYNOTIF_notification_type_wc_customer_on_hold_order extends VOYNOTIF_notification_type {

    function __construct() {
        $this->name = 'wc_customer_on_hold_order';
        $this->label = __( 'Customer on-hold order', 'notifications-center' );
        $this->tags = array('woocommerce');
        parent::__construct();
    }

    function init() {
        add_action( 'woocommerce_order_status_on-hold', array( $this, 'send' ), 10, 1 );
    }
...
new VOYNOTIF_notification_type_wc_customer_on_hold_order();
